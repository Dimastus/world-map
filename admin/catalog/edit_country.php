		  ...
		  echo '
			...
			echo "<table class='table'>
					<thead>
						...
						$query_edit_country = "SELECT `table_country`.* FROM `table_country` JOIN `table_for_tc-tu` ft1  ON ft1.`id_table_country` = `table_country`.`id_country` JOIN `table_users` t1  ON t1.`id` = ft1.`id_table_users` JOIN `table_for_tc-tu` ft2  ON ft2.`id_table_country` = `table_country`.`id_country` JOIN `table_users` t2  ON t2.`id` = ft2.`id_table_users` AND t2.`id` = '" . $code . "' GROUP BY `table_country`.`name_country`"; //запрос выборки данных из БД
			        	$result_edit_country = $connection -> query($query_edit_country);//отправка запроса к MySQL
			        	if(!$result_edit_country) die ("<div class='alert alert-danger'>Сбой при доступе к БД: " . $connection -> error . "</div>");//в случае ошибки извлечения данных - вывод сообщения
			        	else {
			        		$rows_edit_country = $result_edit_country -> num_rows;
							for($j = 0; $j < $rows_edit_country; ++$j)
						    {
						        $result_edit_country -> data_seek($j);
						        $row_edit_country = $result_edit_country -> fetch_array(MYSQLI_NUM);//получение отдельной строки таблицы
		      	...
		      	<form action='catalog/insert_to_db.php' method='post'>
					<div class='input-group mb-3'>
					  	<select class=\"form-control custom-select\" name='sel_country'>";
				    	  	$query_undef_country = "SELECT `name_country`, `id_country` FROM `table_country` LEFT JOIN `table_for_tc-tu` ON `table_country`.`id_country` = `table_for_tc-tu`.`id_table_country` WHERE `table_for_tc-tu`.`id_table_country` IS NULL"; 

				        	$result_undef_country = $connection -> query($query_undef_country);
